...
namespace OurProject_Part1
{
    public partial class Form2 : Form
    {
        ...
        public Form2(ref DataSet ds)
        {
            ...
            dataGridView.DataSource = dsz.Tables["Заказчики"];
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataRow newrow = dsz.Tables["Заказчики"].NewRow();
                // заполнение атрибутов заказчика
                if (textBox1.Text.CompareTo("0") == 0)
                    newrow["IDЗаказчика"] = dsz.Tables["Заказчики"].Rows.Count + 1;
                else
                    newrow["IDЗаказчика"] = int.Parse(textBox1.Text);
                newrow["ФИОЗаказчика"] = (string)textBox2.Text;
                newrow["УчебноеЗаведение"] = (string)textBox3.Text;
                newrow["Связь"] = (string)textBox4.Text;

                // записываем созданную запись в таблицу
                dsz.Tables["Заказчики"].Rows.Add(newrow);
            ...
            {
                MessageBox.Show("Данный заказчик уже существует в базе данных!");
            }
        }
        ...
        {   
            ...
            int nom = (int)dataGridView.SelectedRows[0].Cells["IDЗаказчика"].Value;

            // выбираем все записи, соответствующие текущему заказу
            DataRow[] drs = dsz.Tables["Заказчики"].Select("IDЗаказчика=" + nom);

            // удаляем все найденные заказы из таблиц набора данных
            //for (int i = drs.Length - 1; i >= 0; i--)
            //  drs[i].Delete();
            // поиск заказа по ключу
            dsz.Tables["Заказчики"].Rows.Find(new object[] { (object)nom}).Delete();
