...
namespace MyShop
{
    class Program
    {
        static string connectionString = ConfigurationManager.ConnectionStrings["MyShop.Properties.Settings.TorgovlyaConnectionString"].ConnectionString;

        static SqlDataAdapter daTovar;
        static SqlDataAdapter daPokupatel;
        static SqlDataAdapter daZakaz;
        // Загрузка в DataSet
        public static void BuildDataSet(ref SqlConnection connect, ref DataSet ds, ref SqlDataAdapter[] adapt)
        {
            SqlCommand[] commands = new SqlCommand[3];
                        
            commands[0] = new SqlCommand("SELECT * from Tovar", connect);
            daTovar = new SqlDataAdapter(commands[0]);
            daTovar.FillSchema(ds, SchemaType.Source, "Tovar");
            daTovar.Fill(ds, "Tovar");
            adapt[0] = daTovar;

            commands[1] = new SqlCommand("SELECT * from Pokupatel", connect);
            daPokupatel = new SqlDataAdapter(commands[1]);
            daPokupatel.FillSchema(ds, SchemaType.Source, "Pokupatel");
            daPokupatel.Fill(ds, "Pokupatel");
            adapt[1] = daPokupatel;

            commands[2] = new SqlCommand("SELECT * from Zakaz", connect);
            daZakaz = new SqlDataAdapter(commands[2]);
        ...
        public static void FillDataSet(ref DataSet ds, ref SqlDataAdapter[] adapt)
        {
        ...
        public static void AddToTableAndSave(DataSet ds, SqlDataAdapter adapter, string tableName)
        {
        ...
        static void Main(string[] args)
        {
            ...
            SqlDataAdapter[] adapt= new SqlDataAdapter [3];
            // Sql - то же самое, что и OleDb, но для локальной базы данных
            SqlConnection connect = new SqlConnection(connectionString);
            connect.Open();

            // Загрузка из sql в dataset
        ...
        // Работа с товаром
        public static void RabotaSTovarom(ref DataSet ds,ref SqlDataAdapter[] adapt)
        {
        ...
        // Работа с товаром
        public static void RabotaSZakazom(ref DataSet ds,ref SqlDataAdapter[] adapt)
        {
            DataTable dtTovar = ds.Tables["Tovar"];
            DataTable dtZakaz = ds.Tables["Zakaz"];
