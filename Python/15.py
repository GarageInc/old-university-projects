...
plb.title("Hilbert N (phase) = " + str(N))
...
plb.figure()
plb.plot(w[:-1],gd)
plb.title("Hilbert N (group) = " + str(N))

# результат фильтрации
s1 = sgn.lfilter(b, 1, s) #примен фильтр
s2 = sgn.filtfilt(b,1,s)

plb.figure()
plb.plot(t,s, color='blue', label='Original')
plb.plot(t,s1, color='red',label='Filtered_1')
plb.plot(t,s2, color='green',label='Filtered_2')
plb.legend()
plb.title("Filter result")